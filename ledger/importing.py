    ...
    with db:
        for line, raw_row in enumerate(reader, 2):
            try:
                ...
                # A bad data row must not discard good rows (BUSINESS_RULES.md).
                # Normalizing here, inside the per-row try/except, keeps one
                # invalid row from aborting the whole import.
                result['rejected'] += 1
